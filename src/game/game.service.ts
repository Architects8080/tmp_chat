...
export class GameService {
  ...
  accept(userId: number, roomId: number) {
    ...
    if (gameRoom.player1.id == userId)
      gameRoom.player1.isAccept = true;
    if (gameRoom.player2.id == userId)
      gameRoom.player2.isAccept = true;
    if (gameRoom.player1.isAccept && gameRoom.player2.isAccept)
      return gameRoom;
    return ;
  }
  ...
  cancel(userId: number, roomId: number) {
    ...
    if (gameRoom.player1.id == userId)
      canceledPlayerId = gameRoom.player2.id;
